...
class MakeDataset:
    ...
    def load_and_save(self, url, filename):
        ...
        data.to_csv(os.path.join(raw_data_path, str(filename) + '.csv'), index=False)
...
if __name__ == "__main__":
    ...
    logger.info('Train data loading completed and data saved to the directory Data/raw')
    logger.info('Loading of test data started.')
    md.load_and_save(test_data_url, 'test')
    logger.info('Test data loading completed and data saved to the directory Data/raw')